    ...
    return Bluebird.map(fics, fic => {
      ...
      return Bluebird.map(fic.chapters, chapterInfo => {
        try {
          ...
          const resolveFrom = tracker ? progress.completeWorkWhenResolved(chapterContent, tracker) : chapterContent
          return resolveFrom.then(chapter => {
            process.emit('debug', `Got content for #${chapterInfo.order}: ${chapterInfo.name}`)
            if (tracker) progress.show(fic.title, `${chapterInfo.name}`)
