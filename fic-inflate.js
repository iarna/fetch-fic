  ...
  return fic.then(fic => {
    ...
    return Bluebird.map(fics, fic => {
      ...
      return Bluebird.map(fic.chapters, chapter => {
        ...
        return progress.completeWorkWhenResolved(tracker, chapterContent).then(content => {
          ...
          if (chapter.fetchFrom == null && chapter.link !== content.finalUrl) chapter.fetchFrom = content.finalUrl
